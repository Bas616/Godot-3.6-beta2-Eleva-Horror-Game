...
onready var player = get_tree().get_root().find_node("Player2", true, false)
onready var camera = player.get_node("Player2/Head/Camera")
onready var shootbtn = $"../../../TouchScreenButton"
func _ready():
...
func _process(delta):
	...
	reload_tip()
	if singleshot:
		if Input.is_mouse_button_pressed(BUTTON_LEFT) or shootbtn.is_pressed():
			can_shoot = false
		else:
			can_shoot = true
...
func weapon_bobbing_animation():
	...
	var animation_value = player.player_speed / 600
